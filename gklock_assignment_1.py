...
def import_data(filename):
    ...
    for line in f:
        ...
        split_list= stripped_line.split(",")
        split_list[2]=float(split_list[2])
...
def fight(participant1, participant2, first2attack):
    ...
    base_attack1 = participant1[3]
    base_attack2 = participant2[3]
    while variable1>0 and variable2>0:
        if first2attack==1:
            variable2= variable2- (base_attack1*(attack_multiplier(participant1[1],participant2[1])))
            rounds=rounds+1
            first2attack=2
        elif first2attack==2:
            variable1=variable1-(base_attack2*(attack_multiplier(participant2[1],participant1[1])))
            rounds=rounds+1
            first2attack=1
    if variable1>0:
        #print("The Winner is participant 1:", participant1[0])
        #print("Rounds:", rounds)
        winner=participant1[0]
    elif variable2>0:
        #print("The Winner is participant 2:", participant2[0])
        #print("Rounds:", rounds)
...
def tournament(participants):
    ...
    while i<6:
        ...
        #print(home_fight)
        if home_fight[0] == participants[i][0]:
            #print(participants[i][0], "won at home")
            wins[i]=wins[i]+1
        elif home_fight[0] == participants[j][0]:
            #print(participants[j][0], "won at home")
            wins[j]=wins[j]+1
        away_fight = fight(participants[i],participants[j],2)
        #print (away_fight[0])
        if away_fight[0] == participants[i][0]:
            #print(participants[i][0], "won away")
            wins[i]=wins[i]+1
        elif away_fight[0] == participants[j][0]:
